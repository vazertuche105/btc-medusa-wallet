...
public class UtxosController extends WalletFormController implements Initializable {
    ...
    public void initializeView() {
        ...
        importPrivacy.setTooltip(new Tooltip("Import KYC labels from the Privacy tab into the UTXO Label column"));
        updateImportPrivacyButton();
    ...
    /**
     * Pull KYC labels from the Privacy tab's shared registry and apply them to
     * the matching UTXO entries. Setting the labelProperty fires
     * WalletEntryLabelsChangedEvent automatically, which persists the label.
     */
    public void importPrivacy(ActionEvent event) {
        if(!PrivacyController.hasPrivacyLabels()) {
            return;
        }
        int applied = 0;
        for(Entry entry : getWalletForm().getWalletUtxosEntry().getChildren()) {
            ...
            String txid = utxoEntry.getHashIndex().getHash().toString();
            int vout = (int) utxoEntry.getHashIndex().getIndex();
            String label = PrivacyController.getPrivacyLabel(txid, vout);
            if(label == null || label.isBlank() || "—".equals(label)) {
                continue;
            }
            utxoEntry.labelProperty().set(label);
            applied++;
        }
        log.info("Imported {} privacy labels into UTXOs tab", applied);
    }

    /**
     * Hide the Import Privacy button entirely if no privacy data has been
     * collected yet. Enable it only if at least one of this wallet's UTXOs
     * has a matching label.
     */
    private void updateImportPrivacyButton() {
        if(importPrivacy == null) {
            return;
        }
        if(!PrivacyController.hasPrivacyLabels()) {
            importPrivacy.setVisible(false);
            importPrivacy.setManaged(false);
            return;
        }
        boolean anyMatch = false;
        WalletUtxosEntry walletUtxosEntry = getWalletForm().getWalletUtxosEntry();
        if(walletUtxosEntry != null && walletUtxosEntry.getChildren() != null) {
        ...
        importPrivacy.setVisible(true);
        importPrivacy.setManaged(true);
        importPrivacy.setDisable(!anyMatch);
    }

    @Subscribe
    public void privacyLabelsUpdated(PrivacyLabelsUpdatedEvent event) {
        Platform.runLater(this::updateImportPrivacyButton);
    }
