...
public class AppController implements Initializable {
    ...
    private void openWallet(Storage storage, WalletAndKey walletAndKey, AppController appController, boolean forceSameWindow) {
        try {
            ...
            for(Map.Entry<WalletAndKey, Storage> entry : walletAndKey.getChildWallets().entrySet()) {
                // Skip the BTC Medusa payment account on wallet startup UNLESS
                // there is a pending payment that needs it for Electrum sync.
                if("BTC Medusa".equals(entry.getKey().getWallet().getName())
    ...
    public WalletForm addWalletSubTab(TabPane subTabs, Storage storage, Wallet wallet) {
        try {
            ...
            // Don't auto-focus the BTC Medusa payment account tab — it exists
            // for Electrum sync but should stay in the background.
            if(!"BTC Medusa".equals(wallet.getName())) {
                subTabs.getSelectionModel().select(subTab);
            }
    ...
    private void setMedusaIcon() {
        MedusaStatusLabel medusaLabel = null;
        for(Node node : statusBar.getRightItems()) {
            if(node instanceof MedusaStatusLabel) {
                medusaLabel = (MedusaStatusLabel)node;
            }
        }

        if(medusaLabel == null) {
            medusaLabel = new MedusaStatusLabel();
            // Insert before the server toggle (last item) and before Tor icon if present
            statusBar.getRightItems().add(Math.max(statusBar.getRightItems().size() - 1, 0), medusaLabel);
        } else {
            medusaLabel.update();
        }
    }
    ...
    @Subscribe
    public void medusaStatusTextChanged(MedusaStatusTextEvent event) {
        for(Node node : statusBar.getRightItems()) {
            if(node instanceof MedusaStatusLabel) {
                ((MedusaStatusLabel)node).setStatusText(event.getText());
            }
        }
    }
    ...
    public void newWalletTransactions(NewWalletTransactionsEvent event) {
        // Suppress desktop notifications for the BTC Medusa payment account —
        // its initial sync discovers historical transactions that confuse the user.
        if("BTC Medusa".equals(event.getWallet().getName())) {
            return;
        }
    ...
    public void childWalletsAdded(ChildWalletsAddedEvent event) {
        ...
        for(Wallet childWallet : event.getChildWallets()) {
            // Always create the tab — addWalletTab initialises the WalletForm,
            ...
            // wallet never syncs and never receives transaction events.
